...
$('.add-laporan').submit(function (e) {
    if($('#building').val()==''){    
        ...
        e.preventDefault();
        $.ajax({
            url:"mod/laporan/proses.php?action=add",
            type: "POST",
            ...
            success: function (data) {
                if (data == 'success') {
                   ...
                   window.setTimeout(window.location.href = "./laporan",2500);
                } else {
...
/* -------------------- Edit ------------------- */
$('.update-laporan').submit(function (e) {
    if($("#building").val()==""){    
         swal({title: 'Oops!', text: 'Harap bidang inputan tidak boleh ada yang kosong.!', icon: 'error', timer: 1500,});
         loading();
        return false;
        ...
        e.preventDefault();
        $.ajax({
            url:"mod/laporan/proses.php?action=update",
            type: "POST",
            data: new FormData(this),
            processData: false,
            contentType: false,
...
$('.update-password').submit(function (e) {
    if($("#password").val()==""){    
        ...
        e.preventDefault();
        $.ajax({
            url:"mod/laporan/proses.php?action=update-password",
            type: "POST",
            data: new FormData(this),
            processData: false,
            contentType: false,
        ...
        var id = $(this).attr("data-id");
          ...
          .then((value) => {
            if(value) {
                loading();
                $.ajax({  
                     url:"mod/laporan/proses.php?action=delete",
                     type:'POST',    
                     data:{id:id},  
                    success:function(data){ 
                        if (data == 'success') {
